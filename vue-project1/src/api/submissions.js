  ...
  const poll = async () => {
    ...
    try {
      ...
      if (normalList.length > 0) {
        ...
        if (submitToken) {
          // 如果有submitToken，优先匹配本次提交
          targetSubmission = normalList.find(item => item.submitToken === submitToken)
          // 如果找到了匹配的记录，检查状态
          if (targetSubmission) {
            const status = targetSubmission.status
            if (status !== 'Pending' && status !== 'Judging') {
              // 判题完成，回调结果
              if (onResult) onResult(targetSubmission)
              return
            }
          }
          // 如果没有找到匹配的记录，继续轮询（不使用旧记录）
        } else {
          // 如果没有submitToken，取最新的记录
          targetSubmission = normalList[0] // 后端默认按时间倒序，取最新正常记录
          const status = targetSubmission.status
          if (status !== 'Pending' && status !== 'Judging') {
            // 判题完成，回调结果
            if (onResult) onResult(targetSubmission)
