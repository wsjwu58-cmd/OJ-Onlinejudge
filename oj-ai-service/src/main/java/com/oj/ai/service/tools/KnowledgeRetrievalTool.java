...
/**
 * 知识检索工具 - 使用本地 KnowledgeRetrievalService（InMemoryEmbeddingStore）
 */
@Component
@Slf4j
public class KnowledgeRetrievalTool {
    ...
    public String searchKnowledge(
        ...
        log.info("Tool调用: searchKnowledge, query={}", query);
        try {
            if (topK == null || topK < 1) topK = 3;
            else if (topK > 5) topK = 5;

            List<String> knowledgeList = knowledgeRetrievalService.retrieveKnowledge(
                    query, context != null ? context : "", topK);
    ...
    public String searchProblemKnowledge(
        ...
        log.info("Tool调用: searchProblemKnowledge, problemId={}", problemId);
        if (problemId == null) {
            throw new ParameterMissingException("problemId");
        }
        try {
            String context = "题目ID: " + problemId;
            List<String> knowledgeList = knowledgeRetrievalService.retrieveKnowledge(question, context, 3);
    ...
    public String searchErrorSolution(
        ...
        log.info("Tool调用: searchErrorSolution, errorInfo={}", errorInfo);
        try {
