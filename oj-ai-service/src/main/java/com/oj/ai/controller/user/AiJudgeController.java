...
public class AiJudgeController {
    ...
    public Result<String> submitJudge(@Valid @RequestBody AiJudgeDTO dto) {
        ...
        stringRedisTemplate.opsForValue().set(AI_JUDGE_PREFIX + token, JSON.toJSONString(dto), TASK_EXPIRE_MINUTES, TimeUnit.MINUTES);
    ...
    public SseEmitter streamJudge(@PathVariable String token) {
        ...
        if (json == null) {
            ...
            try { emitter.send(SseEmitter.event().data("任务不存在或已过期")); emitter.complete(); } catch (IOException e) { emitter.completeWithError(e); }
            return emitter;
        }
    ...
    public Result<String> submitSyntaxCheck(@Valid @RequestBody AiJudgeDTO dto) {
        String token = UUID.randomUUID().toString();
        stringRedisTemplate.opsForValue().set(AI_SYNTAX_PREFIX + token, JSON.toJSONString(dto), TASK_EXPIRE_MINUTES, TimeUnit.MINUTES);
        return Result.success(token);
    }
    ...
    public Result<String> submitAnalyzeError(@Valid @RequestBody AiJudgeDTO dto) {
        String token = UUID.randomUUID().toString();
        dto.setUserId(BaseContext.getCurrentId());
        stringRedisTemplate.opsForValue().set(AI_ERROR_PREFIX + token, JSON.toJSONString(dto), TASK_EXPIRE_MINUTES, TimeUnit.MINUTES);
        return Result.success(token);
    }
    ...
    public SseEmitter streamAnalyzeError(@PathVariable String token) {
        ...
        AiJudgeDTO dto = JSON.parseObject(json, AiJudgeDTO.class);
        return aiJudgeService.analyzeError(dto, null);
    }
    ...
    public Result<String> submitChat(@RequestBody AiChatDTO dto) {
        String token = UUID.randomUUID().toString();
        stringRedisTemplate.opsForValue().set(AI_CHAT_PREFIX + token, JSON.toJSONString(dto), TASK_EXPIRE_MINUTES, TimeUnit.MINUTES);
        return Result.success(token);
    }
    ...
    public Result<String> submitHint(@RequestBody AiChatDTO dto) {
        String token = UUID.randomUUID().toString();
        stringRedisTemplate.opsForValue().set(AI_HINT_PREFIX + token, JSON.toJSONString(dto), TASK_EXPIRE_MINUTES, TimeUnit.MINUTES);
        return Result.success(token);
    }
